...
export default function ManageResourcesPage({ title, endpoint, fields }) {
  const { id } = useParams();
  const BASE_URL = import.meta.env.VITE_BASE_URL;

  const [rows, setRows] = useState([Object.fromEntries(fields.map(f => [f.name, ""]))]);
  const [existing, setExisting] = useState([]);
  const [loading, setLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  const addRow = () => setRows([...rows, Object.fromEntries(fields.map(f => [f.name, ""]))]);
  ...
  const displayed = existing.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return (
    <div className="bg-[#0e1525] text-gray-100 min-h-screen flex flex-col">
      ...
      <main className="flex-1 mt-[72px] p-10 space-y-8">
        <h1 className="text-3xl font-bold tracking-wide mb-6">{title} for Server #{id}</h1>
          ...
          <form onSubmit={handleSubmit}>
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#151c2f] text-gray-300 uppercase text-sm tracking-wider">
                <tr>
                  {fields.map(f => (
                    <th key={f.name} className="px-6 py-3 border-b border-indigo-900/40">
                      {f.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-[#141b2e]" : "bg-[#19223c]"}>
                    {fields.map(f => (
                      <td key={f.name} className="px-6 py-3 border-b border-indigo-900/30">
                        {f.type === "checkbox" ? (
                          <input
                            type="checkbox"
                            checked={row[f.name]}
                            onChange={(e) => handleChange(i, f.name, e.target.checked)}
        ...
        {/* Existing Table */}
        <div className="bg-gradient-to-br from-[#151c2f] to-[#1e2640] rounded-2xl p-6 shadow-2xl border border-indigo-900/40">
          <h2 className="text-2xl font-semibold text-indigo-400 mb-4">Existing Entries</h2>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#151c2f] text-gray-300 uppercase text-sm tracking-wider">
                  <tr>
                    {Object.keys(displayed[0]).map((key) => (
                      <th key={key} className="px-6 py-3 border-b border-indigo-900/40">
                        {key}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {displayed.map((item, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-[#141b2e]" : "bg-[#19223c]"}>
                      {Object.values(item).map((val, j) => (
                        <td key={j} className="px-6 py-3 border-b border-indigo-900/30">
                          {String(val)}
                        </td>
                      ))}
                    </tr>
