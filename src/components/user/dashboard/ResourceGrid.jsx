...
const ResourceGrid = ({ statsData }) => {
  ...
  const handleCardClick = (res) => {
    if (res.isComingSoon) return; // Firewall → no action
    navigate("/orders");
  };
  if (!statsData) {
    return (
            ...
            <div
              ...
              className={`p-4 rounded-xl border transition-all duration-300 hover:scale-[1.02] ${
                res.isComingSoon
                  ...
                  : "cursor-pointer border-gray-800 hover:border-indigo-500/50"
              } ${colors.bg}`}
            >
              <div className="flex items-center gap-3 mb-2">
                <div
                  className={`p-2 rounded-lg ${colors.iconBg} ${colors.iconText}`}
                >
                  {res.icon}
                </div>
              ...
              {!res.isComingSoon &&
                res.type === "ip" &&
                statsData.ipAddresses &&
                statsData.ipAddresses.length > 0 && (
                  <div className="mt-3">
